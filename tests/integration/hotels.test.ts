...
describe('GET /hotels', () => {
    ...
    describe('When token is valid', () => {
        ...
        it('Should response with status 402 there is payment but ticket does not includes hotel', async () => {
            const user = await createUser();
            const token = await generateValidToken(user);
            const enrollment = await createEnrollmentWithAddress(user);
        ...
        it('Should response with status 402 there is payment but tickcet is remote', async () => {
            const user = await createUser();
            const token = await generateValidToken(user);
            const enrollment = await createEnrollmentWithAddress(user);
